...
void TcpClient::DoRecvMessage(SocketPackageData *packageData) {
    //ALOGI("LCA Report type=0x%04x,size = %d\n", packageData->type, packageData->dataSize);
    bool report = false;
    char buffer[64];
    ...
    switch (packageData->type) {
        ...
        case TYPE_SOCKET_HEARTBEAT:
			...
			//ALOGI("LCA ID = %d RID = %d recv HEARTBEAT ...\n",hb.id,hb.rid);
            if (hb.rid == 0) {
